    ...
    S::Item: AddFakeLag,
{
    pub fn new(config: Config, orig_rx: S, new_tx: mpsc::UnboundedSender<S::Item>) -> Self {
        let lag_distribution =
            rand_distr::Normal::new(config.lag_mean.as_secs_f32(), config.lag_std_dev).unwrap();
        ...
        loop {
            select! {
                message = orig_rx.next().fuse() => {
                    debug!("got original message");
                    let mut rng = rand::thread_rng();

                    match message {
                        Some(mut message) => {
                            if rng.gen::<f32>() >= self.config.loss {
                                let lag = Duration::from_secs_f32(self.lag_distribution.sample(&mut rng));
                                debug!("delaying by {:?}", lag);
