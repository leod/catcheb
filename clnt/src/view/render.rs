...
use crate::view::{stage::Stage, Resources};
...
pub fn render_game(
    ...
    stage.plain.push_quad(
        &AaRect::from_top_left(Point2::origin(), state.settings.map.size).into(),
        0.0,
        Color4::new(0.2, 0.2, 0.2, 1.0),
    );
...
fn rect_to_transform(rect: &geom::Rect) -> Matrix3<f32> {
    Matrix3::new(
        rect.x_edge.x,
        rect.y_edge.x,
        rect.center.x,
